...
export function useDragHistory(maxHistory = 10) {
  ...
  const addToHistory = useCallback((operation: DragOperation) => {
    historyRef.current = [operation, ...historyRef.current.slice(0, maxHistory - 1)]
  }, [maxHistory])
  
  const undo = useCallback(async (onUndo: (operation: DragOperation) => Promise<void>) => {
