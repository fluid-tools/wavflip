  ...
  const renderSoundCard = (sound: GeneratedSound) => {
    ...
    return (
      <ContextMenu key={sound.id}>
        <ContextMenuTrigger>
          <div 
            className="group bg-neutral-900/50 backdrop-blur-sm rounded-xl border border-neutral-800 hover:bg-neutral-800/80 transition-all duration-200 cursor-pointer hover:border-neutral-700 shadow-sm hover:shadow-md mx-4 mb-3"
            onClick={() => onPlaySound(sound)}
          >
            <div className="p-4">
              <div className="flex items-start gap-3 mb-3">
                <div className="flex items-center justify-center w-10 h-10 rounded-lg bg-neutral-800 border border-neutral-700 flex-shrink-0">
              ...
              {/* Waveform Preview */}
              <div className="mb-3">
                <WaveformPreview url={sound.url} height={28} />
              </div>
    ...
    <Sheet>
      ...
      <SheetContent side="right" className="w-[420px] p-0">
        ...
        <div className="h-full">
          <Virtuoso
            style={{ height: '100%' }}
            totalCount={generatedSounds.slice(0, 20).length}
            itemContent={(index) => renderSoundCard(generatedSounds.slice(0, 20)[index])}
            components={{
              List: ({ style, children }) => (
                <div style={{ ...style, paddingTop: '16px', paddingBottom: '16px' }}>
                  {children}
                </div>
              )
            }}
