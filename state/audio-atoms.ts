...
export const generatedSoundsAtom = atom<GeneratedSound[]>((get) => {
  ...
  return queue.tracks.filter((track): track is GeneratedSound => track.type === 'generated')
})
      ...
      case 'TOGGLE_SHUFFLE':
        set(queueAtom, (prev) => {
          if (prev.shuffleOrder) {
            ...
            // Enable shuffle
            const shuffleOrder = generateShuffleOrder(prev.tracks.length, prev.currentIndex)
