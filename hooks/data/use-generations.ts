...
export function useGenerations() {
  ...
  const saveOffline = useMutation({
    mutationFn: async (sound: GeneratedSound) => {
      ...
      const normalized: GeneratedSound = {
        ...sound,
        id: sound.key || sound.id,
      };
      ...
      queryClient.setQueryData(
        generationsKeys.localCache(),
        (prev: Map<string, LocalVaultTrack> | undefined) => {
  ...
  generations.sort(
    (a: GeneratedSound, b: GeneratedSound) =>
...
export function useGenerationActions() {
  ...
  const { executeAsync: execSFX, isPending: isSFXPending } =
    useAction(generateSoundEffect);
  const { executeAsync: execTTS, isPending: isTTSPending } =
