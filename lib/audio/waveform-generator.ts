...
export async function generateWaveformData(
  ...
  const AudioContextClass: AudioContextCtor | undefined = (w.AudioContext ||
    w.webkitAudioContext) as AudioContextCtor | undefined;
  if (!AudioContextClass) {
    throw new Error(
      'Web Audio API is not supported in this browser/environment.'
    );
  }
...
export function generatePlaceholderWaveform(duration: number): WaveformData {
  ...
  for (let i = 0; i < peaksCount; i++) {
    ...
    const amplitude =
      0.3 + 0.2 * Math.sin(t * Math.PI * 4) + 0.1 * Math.random();
